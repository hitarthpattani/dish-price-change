...
| 4   | Types split                 | Domain interfaces inlined (`ReportRow` in builder; nav interfaces in `NavigationProvider/index.tsx`)                                                                | Split exported domain interfaces into sibling `types.ts` (`report-builder/types.ts`, `MainPage/utils/NavigationProvider/types.ts`). Small local interface (`Logger`) left inline | `06`, `07-admin-ui-scaffold.md`                    |
...
`MainPage/utils/NavigationProvider/types.ts`), matching `collection/<name>/types.ts`. A small,
tightly-coupled
...
- `MainPage/utils/NavigationProvider/types.ts` adds
