 ...
 *
 * Alarm-triggered cron (web: 'no'; empty HTTP-method list). Purges complete notification rows.
...
export const main = RuntimeAction.execute('remove-renewal-notifications', [], [], [], async () => {
  ...
  //   - Delete COMPLETE rows older than `remove_renewal_day_interval` days, capped by `remove_record_batch`.
  //
  // Foundation artifacts to call:
  //   - lib/database/repository/prerenewal-notifications → deleteCompletedOlderThan(days, batchSize)
  //   - lib/utils/logger
